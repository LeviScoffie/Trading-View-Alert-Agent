...
class ReportGenerator:
    ...
    def generate_weekly_report_data(self, report_date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        # Find most recent Sunday (weekday: Mon=0 ... Sat=5, Sun=6 → days back: Mon=1 ... Sat=6, Sun=0)
    ...
    def generate_context_analysis(
        ...
        # Determine dominant pattern direction for alignment scoring
        ...
        # Check multi-timeframe alignment against dominant direction
        tf_alignment = 0
        tf_count = 0
        for tf_name, tf_data in multi_tf.items():
            if tf_data.get('trend') in ['bullish', 'bearish']:
                tf_count += 1
                if dominant_direction and tf_data.get('trend') == dominant_direction:
                    tf_alignment += 1
