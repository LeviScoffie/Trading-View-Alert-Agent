...
def cleanup_old_data():
    ...
    # --- Clean alerts and behavior_logs from the webhook-receiver database ---
    alerts_conn = sqlite3.connect(ALERTS_DB_PATH)
    try:
        if "alerts" in CLEANUP_QUERIES:
            query = CLEANUP_QUERIES["alerts"].format(retention_days=retention_days)
            cursor = alerts_conn.execute(query)
            deleted_counts["alerts"] = cursor.rowcount
            logger.info(f"Deleted {cursor.rowcount} old alerts from {ALERTS_DB_PATH}")
        ...
        if "behavior_logs" in CLEANUP_QUERIES:
            ...
            logger.info(f"Deleted {cursor.rowcount} old behavior logs from {ALERTS_DB_PATH}")

        alerts_conn.commit()
    except Exception as e:
        alerts_conn.rollback()
        logger.error(f"Error cleaning alerts DB: {e}")
        raise
    finally:
        alerts_conn.close()

    # --- Clean job execution history from the scheduler's own database ---
    sched_conn = sqlite3.connect(DATABASE_PATH)
    try:
        if "job_executions" in CLEANUP_QUERIES:
            query = CLEANUP_QUERIES["job_executions"].format(retention_days=retention_days)
            cursor = sched_conn.execute(query)
            deleted_counts["job_executions"] = cursor.rowcount
            logger.info(f"Deleted {cursor.rowcount} old job execution records from {DATABASE_PATH}")

        sched_conn.commit()
    except Exception as e:
        sched_conn.rollback()
        logger.error(f"Error cleaning scheduler DB: {e}")
        raise
    finally:
        sched_conn.close()
