...
class TradingViewAlert(BaseModel):
    ...
    Standard fields are mapped from TradingView template variables:
        symbol   → {{ticker}}
        open     → {{open}}
        high     → {{high}}
        low      → {{low}}
    ...
    Recommended TradingView alert message template:
        {
          "symbol":   "{{ticker}}",
          "open":     {{open}},
          "high":     {{high}},
    ...
    message:  Optional[str]  = Field(None, description="Alert message / strategy action")
    time:     Optional[str]  = Field(None, description="Alert timestamp from TradingView")
    interval: Optional[str]  = Field(None, description="Chart interval, e.g. 1D, 4H")

    # OHLCV fields — present when alert message template includes them
    open:   Optional[float] = Field(None, description="Open price of the alert candle")
    high:   Optional[float] = Field(None, description="High price of the alert candle")
    low:    Optional[float] = Field(None, description="Low price of the alert candle")
    close:  Optional[float] = Field(None, description="Close / current price")
    volume: Optional[float] = Field(None, description="Volume of the alert candle")

    # Legacy convenience alias kept for backwards compatibility
    price: Optional[float] = Field(None, description="Alias for close price (legacy)")

    # Allow any extra fields TradingView may add in the future
    class Config:
        extra = "allow"
    ...
    def effective_price(self) -> Optional[float]:
        """Return the best available price: close → price → None."""
        return self.close or self.price

    def ohlcv_candle(self) -> Optional[Dict[str, Any]]:
        """Return a candle dict if all OHLCV fields are present, else None."""
        if all(v is not None for v in (self.open, self.high, self.low, self.close)):
            return {
                "timestamp": self.time or datetime.utcnow().isoformat(),
                "open":   self.open,
                "high":   self.high,
                "low":    self.low,
                "close":  self.close,
...
async def receive_webhook(
    ...
    logger.info("Received webhook from %s — symbol=%s interval=%s",
                client_ip, alert.symbol, alert.interval or "unknown")
    ...
    try:
        ...
        logger.error("Failed to store alert: %s", str(e))
        raise HTTPException(
...
async def receive_webhook_alt(
    ...
    return await receive_webhook(alert, request, background_tasks)
...
async def get_recent_analysis(limit: int = 20):
    """Return the most recent analysis results across all symbols."""
    results = db.get_analysis_results(limit=limit)
    return {"count": len(results), "results": results}


@app.get("/analysis/{symbol}")
async def get_analysis_by_symbol(symbol: str, limit: int = 10):
