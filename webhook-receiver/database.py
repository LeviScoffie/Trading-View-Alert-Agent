...
class AlertDatabase:
    ...
    def _init_database(self) -> None:
        ...
        with self._get_connection() as conn:
            ...
            # Analysis results — persists every engine output for history & reports
            conn.execute("""
    ...
    def store_analysis_result(
        ...
        """Persist a full analysis engine result.

        Args:
            alert_id:  FK to the originating alert row.
            symbol:    Trading symbol.
            timeframe: TradingView interval string, e.g. "1D".
            result:    Full AnalysisResult serialised as a dict.
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                ...
                INSERT INTO analysis_results
                    (alert_id, symbol, timeframe, confidence, recommendation,
                     patterns_json, ma20_json, context_json, full_result_json)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    alert_id,
            ...
            logger.info("Stored analysis result %d for %s", row_id, symbol)
            return row_id
    ...
    def get_analysis_results(
        ...
        """Retrieve recent analysis results, optionally filtered by symbol."""
        with self._get_connection() as conn:
            if symbol:
                cursor = conn.execute(
                    """
